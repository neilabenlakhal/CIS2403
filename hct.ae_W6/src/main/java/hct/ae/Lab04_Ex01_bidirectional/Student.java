...
will be assigned to the data members */
        ...
        this.gpa = gpa_param;
        this.hctProfile=new HCTProfile();
    }

    // HCTProfile reference
    private HCTProfile hctProfile;

    // Constructors and methods remain the same
    // Getter and Setter for HCTProfile
    public HCTProfile getHctProfile() {
        return hctProfile;
    }

    /**
     * Sets the associated HCTProfile for the student, maintaining bidirectional
     * relationship.
     *
     * @param hctProfile The new HCTProfile to be associated with the student.
     */
    public void setHctProfile(HCTProfile hctProfile) {
        // Check if the new HCTProfile is different from the current associated HCTProfile
        if (this.hctProfile != hctProfile) {
            // Update the student's reference to the new HCTProfile
            this.hctProfile = hctProfile;

            // Establish bidirectional relationship: set the student reference in the new HCTProfile
            hctProfile.setStudent(this);

        }
        // If the new HCTProfile is the same as the current one, no further action is required.
    }
